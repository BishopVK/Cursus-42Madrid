...
static void	check_rgb_values(t_map_array *map_array,
	char **splited, int *count_colors)
{
	int	j;
	int	k;
...
static void	check_element_rgb(t_map_array *map_array, t_rgb *element, int i)
{
	...
	while (element->element[i])
	{
		if (i == 1)
		{
			...
			count_colors = 0;
			check_rgb_values(map_array, splited, &count_colors);
			if (count_colors != 3)
			{
				free_double_pointer(splited);
...
void	check_elements(t_map_array *map_array)
{
	...
	check_element_rgb(map_array, map_array->floor, 0);
	check_element_rgb(map_array, map_array->ceiling, 0);
}
